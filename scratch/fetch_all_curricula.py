...
# Map department names to department codes used in our app
DEPT_NAME_TO_CODE = {
...
for f in fakulteler:
    ...
    for b in r_bol.json():
        ...
        prog_adi_en = b.get("ProgramAdiEn")

        dept_code = DEPT_NAME_TO_CODE.get(prog_adi)
        if not dept_code:
            print(f"Unmapped program: {prog_adi} (ID: {prog_id})")
            continue
        ...
        # We can take the latest curriculum (first one or highest year)
        latest_curr = currs[0]
        curr_id = latest_curr[0]
        curr_name = latest_curr[2]

        # Fetch courses for latest curriculum
        ...
        for s in courses_raw:
            class_no = String = str(s[3] or "").strip()
            ...
            credit = str(s[12] or "").strip()
            ects = str(s[13] or "").strip()
